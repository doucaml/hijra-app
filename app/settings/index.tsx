...
export default function Screen() {
  const [notificationsValue, setNotificationValue] = useState(false);

  const [hijraDateCorrection, setHijraDateCorrection] = useState(0);

  const onDateCorrectionPress = () => {
    setHijraDateCorrection((prev) => {
      if (prev === 2) return -2;
      else return prev + 1;
    });
  };
      ...
      <View>
        ...
        <View className="flex-row items-center justify-between p-2 my-1 h-12 rounded-lg bg-gray-200">
          ...
          <Switch
            ...
            onValueChange={(newValue) => setNotificationValue(newValue)}
          />
        </View>
